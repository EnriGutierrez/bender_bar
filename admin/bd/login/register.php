...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["username"]))) {
    $username_err = "introdusca un usuario.";
  } else {
    ...
    if (!ctype_alnum(str_replace(array("@", "-", "_"), "", $username))) {
      ...
      if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
          ...
          echo "<script>" . "alert('¡Ups! Algo salió mal. Por favor, inténtelo de nuevo más tarde.')" . "</script>";
        }
  ...
  if (empty(trim($_POST["email"]))) {
    $email_err = "Por favor introduzca una dirección de correo eléctronico";
  } else {
    $email = filter_var($_POST["email"], FILTER_SANITIZE_EMAIL);
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
      $email_err = "Por favor, introduce una dirección de correo electrónico válida.";
    } else {
      # Prepare a select statement
  ...
  if (empty(trim($_POST["password"]))) {
    ...
    if (strlen($password) < 8) {
      $password_err = "La contraseña debe contener al menos 8 o más caracteres.";
    }
  }
  ...
  if (empty($username_err) && empty($email_err) && empty($password_err)) {
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
      ...
      # Execute the prepared statement
      if (mysqli_stmt_execute($stmt)) {
        echo "<script>" . "alert('El registro se completó con éxito. Inicie sesión para continuar.');" . "</script>";
